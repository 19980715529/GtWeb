...
import com.smallchill.core.toolbox.kit.ValidatorKit;
...
public abstract class BladeValidator extends BladeInterceptor {
	...
	protected void validateMinInteger(String field, int min,
			String errorMessage) {
		validateMinIntegerValue(request.getParameter(field), min,
				errorMessage);
	}
	...
	private void validateMinIntegerValue(String value, int min,
			String errorMessage) {
		if (StrKit.isBlank(value)) {
			addError(errorMessage);
			return;
	...
	protected void validateMobile(String field, String errorMessage) {
		if(!ValidatorKit.isMobile(field)){
			addError(errorMessage);
		}
	}
	...
	public void validateStringMinLenValue(String value, int minLen,
			String errorMessage) {
		if (StrKit.isBlank(value)) {
			addError(errorMessage);
			return;
