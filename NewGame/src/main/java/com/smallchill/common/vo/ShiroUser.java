...
public class ShiroUser implements Serializable {
	...
	public ShiroUser(Object id, Object deptId, String loginName, String name, List<Integer> roleList) {
		...
		// 判断用户最高权限
		List<Map> selectList = Db.selectList("SELECT tips FROM [RYPlatformManagerDB].[dbo].[blade_role] where id in (#{join(roles)})", CMap.init().set("roles", Convert.toIntArray(roles.toString())));
		for (Map map : selectList) {
			String tips = JSON.toJSONString(map.get("tips")).replaceAll("\"", "");
			if(StrKit.equals(tips, "administrator")) {
				this.isSysRole = true;
				break;
			}
			if(StrKit.equals(tips, "admin")) {
				this.isAdminRole = true;
			}
			if(StrKit.equals(tips, "user")) {
		...
		if (Func.isOracle()) {
			...
			StringBuilder sb = new StringBuilder();
//			for (Integer roleid : roleList) {
				roleSql = "SELECT id FROM [RYPlatformManagerDB].[dbo].[blade_role] where pid in (#{join(roleIds)})";
				List<Map> list = Db.selectList(roleSql, CMap.init().set("roleIds", roleList));
				//String str = Db.queryStr(roleSql, CMap.init().set("roleIds", roleList));
				for (Map m : list) {
					sb.append(m.get("id")).append(",");
				}
//			}
