	...
	contextMenuInit : function(gridtbl, rightMenuHtml, code) {
		...
		var option = {
			...
			bindings : {
				...
				'rightMenu_excel' : function(t) {
					var postdata = $(gridtbl).jqGrid('getGridParam', 'postData');
					var source = (typeof (export_source) == "undefined") ? (code + ".list") : export_source;
					//console.log('postdata: ' + decodeURI(postdata['where']));
					...
					layer.open({
			    	    ...
			    	    fix: false, //不固定
			    	    maxmin: true,
			    	    content: ctx + '/gameExcel?where=' + postdata['where'] + '&source=' +source+'&type='+export_type
			    	});
				}
			}
		};
