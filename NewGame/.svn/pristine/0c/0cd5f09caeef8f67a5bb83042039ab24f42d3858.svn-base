...
public class CacheController extends BaseController {
	...
	public AjaxResult getChildBtn() {
		...
		List<Map> btnList = Db.selectListByCache(SYS_CACHE, BTN_CHILD_LIST + code + "_" + userId, sql.toString(), 
				...
				.set("roleIn", Convert.toIntArray(roleIn))
				.set("roleOut", Convert.toIntArray(roleOut)));
		System.out.println("按钮组第二个=========="+btnList);
		return json(btnList);
	}
					...
					public Object load() {
						...
						if (!Func.isEmpty(pid)) {
							...
							if(ShiroKit.getUser().getIsSysRole()) {
								...
								table = "(select * from BLADE_MENU where id in( select MENUID from BLADE_RELATION where roleId in (#{join(pid)}) ) and rolelevel <> '1')";
							}
						}
						StringBuilder sb = new StringBuilder();
//						sb.append("select m.id \"id\",(select id from BLADE_MENU  where code=m.pCode) \"pId\",name \"name\",(case when m.levels=1 then 'true' else 'false' end) \"open\",(case when r.menuId is not null then 'true' else 'false' end) \"checked\",(case when r.menuId is not null then 'false' else 'true' end) \"nocheck\"");
		...
		List<Map<String, Object>> menu = CacheKit.get(SYS_CACHE, ROLE_TREE + Id,
				new ILoader() {
					public Object load() {
						String exclude = "";
						if(!ShiroKit.getUser().getIsSysRole()) {
							exclude = " where r.ID <> 7 ";
						}
						String sql = "select id \"id\",pId \"pId\",name as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\" from  BLADE_ROLE r "+exclude+" order by pId,num asc";
						if (Id.indexOf(",") == -1) {
							sql = "select r.id \"id\",pId \"pId\",name as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\",(case when (r1.ID is null) then 'false' else 'true' end) \"checked\" from  BLADE_ROLE r left join (select ID  from BLADE_ROLE where ID in ("
									+ "#{join(roleId)}"
									+ ")) r1 on r.ID=r1.ID "+exclude+" order by pId,num asc";
						}
